  ...
  return (
    <div className="min-h-screen safe-top safe-bottom">
      <div className="max-w-2xl mx-auto px-5 py-8">
        <h1 className="text-xl font-semibold mb-6">Listening results</h1>
              ...
              return (
                <div key={q.number} className="flex items-start gap-2 text-xs py-1.5 border-b border-[var(--color-border)] last:border-0">
                  <span
                    ...
                    style={{ background: correct ? "var(--color-success)" : "var(--color-danger)", color: "white" }}
                  >
                    {q.number}
                  </span>
        ...
        <Card className="mb-6">
          <p className="font-medium text-sm mb-3">Transcripts</p>
          <div className="flex flex-col gap-2">
            {test.sections.map((s) => (
              <div key={s.index}>
                <button
                  onClick={() => setShowTranscript(showTranscript === s.index ? null : s.index)}
                  className="text-xs font-medium text-[var(--color-primary-2)]"
                >
                  {showTranscript === s.index ? "Hide" : "Show"} Section {s.index + 1} transcript
                </button>
                {showTranscript === s.index && (
                  <p className="text-xs text-[var(--color-text-muted)] mt-2 whitespace-pre-line leading-relaxed">{s.script}</p>
                )}
              </div>
            ))}
          </div>
