...
import { readingRawToBand } from "@/lib/scoring/band";
...
export async function POST(req: Request, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const band = readingRawToBand(result.rawScore, result.total);

  await prisma.readingAttempt.update({
    where: { id: attemptId },
    data: {
  ...
  await recordSkillScore({ userId, skill: "READING", band, source: attempt.kind, refId: attemptId });
